...
if (isset($_GET['id'])) {
    ...
    $check_query = "SELECT * FROM parents WHERE admission_number = ?";
    $check_stmt = $conn->prepare($check_query);
    $check_stmt->bind_param("s", $parent_id);
    $check_stmt->execute();
    ...
    if ($check_result->num_rows > 0) {
        ...
        echo "<script>alert('No parent found with that admission number.'); window.location.href='manage_parents.php';</script>";
    }
